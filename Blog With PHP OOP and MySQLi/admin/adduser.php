...
<?php  //(from header.php) for protect add user without admin
...
<?php
    if ($_SERVER['REQUEST_METHOD']=='POST') {
        ...
        if(empty($username) || empty($password) || empty($role) || empty($email)){
            ...
            if ($mailcheck != false) {  //if the email's row value is not equal==0,  that means row of email column= 1, which is 1>0 already exist  
